...
const HistoryModal: React.FC<HistoryModalProps> = ({ studentName, history, onClose }) => {
  return (
    <div 
    ...
    >
      <div 
        className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col m-4"
        onClick={e => e.stopPropagation()} // Prevent modal from closing when clicking inside
      >
        <header className="flex justify-between items-center p-4 border-b border-slate-200 dark:border-slate-700">
          <h2 id="history-modal-title" className="text-xl font-bold text-slate-800 dark:text-slate-100">
            {studentName} 학생 변경 이력
          </h2>
